...
# 以hadoop用户身份创建HDFS目录
su - hadoop -c "hadoop fs -mkdir -p /data/input/user_item_scores/dt=\$(date +%F)"
su - hadoop -c "hadoop fs -mkdir -p /data/output"

# 以hadoop用户身份上传评分数据到HDFS
echo "上传评分数据到HDFS..."
su - hadoop -c "hadoop fs -put /tmp/user_item_scores_\$(date +%F).csv /data/input/user_item_scores/dt=\$(date +%F)/part-00000"

# 以hadoop用户身份上传推荐快照到HDFS
echo "上传推荐快照到HDFS..."
su - hadoop -c "hadoop fs -put /tmp/recommendation_snapshots_\$(date +%F).csv /data/output/recommendation_snapshots/part-00000"

# 清理临时文件
rm -f /tmp/user_item_scores_\$(date +%F).csv
rm -f /tmp/recommendation_snapshots_\$(date +%F).csv
