...
def get_all_items(
    ...
    order_by: Optional[str] = Query(None, description="排序方式（web前端）: created_at_desc(最新发布), price_asc(价格从低到高), price_desc(价格从高到低), views_desc(最受欢迎), dynamic_sort(动态排序), bigdata_recommendation(大数据推荐), ai_recommendation(AI增强推荐)"),
    ...
    if order_by == "dynamic_sort":
        ...
        return result
    elif order_by == "ai_recommendation":
        # 使用AI增强推荐排序
        print(f">>>>>>> 进入AI增强推荐分支，用户ID: {user_id}")
        ...
        # 获取AI增强推荐商品
        recommended_items = ai_enhanced_recommendation_service.get_recommended_items(
            db, user_id, limit
        )
        print(f">>>>>>> 获取到 {len(recommended_items)} 个AI增强推荐商品")
        ...
        for item in recommended_items:
            ...
            print(f">>>>>>> 设置AI增强推荐标识: item_id={item.id}, is_ai_recommended={item_dict['is_ai_recommended']}")
